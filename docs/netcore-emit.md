...
### Ограничение записи, о котором нужно знать

Текущий `Post` — **upsert без сравнения содержимого**: повторная присылка объекта с тем же
идентификатором молча перезаписывает существующую строку. Для данных, где повторная отправка
означает сбой клиента или порчу данных, такое поведение опасно: расхождение исчезает бесследно.

Способа выразить «только вставка, конфликт при расхождении» конфигурацией сейчас нет — это
ближайшая задача развития эмита.
