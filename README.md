...
Если в [файле описания объекта](#2-файл-описания-структуры-объекта) (например, `user_item.json`) задано поле `authorize`, оно имеет приоритет над значением в конфиге метода.
...
Уровень прав для маршрутов этого типа данных (anonymous, user, admin, none). Имеет приоритет над полем `authorize` в [описании метода](#1-1-1-описание-структуры-методов-работы-с-объектами-данных-method) в generation_config. По умолчанию none.  
```json
"authorize": "anonymous",
```
