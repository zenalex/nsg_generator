...
2.	Создайте или подготовьте следующие проекты: Flutter - для генерации клиента, ASP.NET – для генерации сервера
...
userSettungs - класс для хранения настроек пользователя
Если не указана, будет сгенерирован обычный класс данных
```json
"entityType": "userSettungs",
```
...
Если тип - [генерируемый объект](#ссылочные-типы-данных), можно указать имя ссылки для Flutter. Значение не должно совпадать с name и referenceName других полей данного объекта. По умолчанию равен name без "Id" в конце  
```json
