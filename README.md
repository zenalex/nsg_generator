...
Встраивать локализацию. По умолчанию false  
```json
"useLocalization": "true",
```

Локаль по умолчанию для проекта  
```json
"defaultLocale": "ru",
```

Применять "новую" логику сохранения табличных частей. При сохранении объекта с таблицами строки
будут удаляться, только если выставить им state = marked. Иначе - все, которые не были переданы. По умолчанию false  
```json
"newTableLogic": "true",
```

Встроить чат поддержки Chatista Connect. По умолчанию выключено. Разворачивается в тип, метод и функцию выдачи токена — см. [чат поддержки](#1-3-чат-поддержки-supportchat)  
```json
...
> **Незнакомые ключи не применяются.** Ключ верхнего уровня, которого генератор не знает, пропускается, а его список печатается предупреждением. Обычно это значит, что сборка генератора старее конфига: настройка написана, но не действует. Отказом это не считается — лишние ключи бывают заведены осознанно.
...
[Уровень прав](#уровни-авторизации), требуемый для обращения к данным. Если в [файле описания объекта](#2-файл-описания-структуры-объекта) (например, `user_item.json`) задано поле `authorize`, оно имеет приоритет над значением в конфиге метода.
...
[Уровень прав](#уровни-авторизации), требуемый для обращения к функции
```json
"authorize": "user",
```
...
### 1. 3. Чат поддержки (supportChat)

Встраивает выдачу токена для чата поддержки Chatista Connect. Одна секция вместо трёх ручных записей: файла описания DTO, регистрации типа в `method` и функции в `functions`.

```json
"supportChat": {
    "enabled": true,
    "productExternalKey": "titan_lk",
    "typeName": "ConnectToken"
}
```

Допускается краткая форма `"supportChat": true` — со значениями по умолчанию.

Включить встраивание. По умолчанию `true`, если секция задана объектом  
```json
"enabled": true,
```

Ключ продукта в тенанте Chatista. По умолчанию равен `applicationName`  
```json
"productExternalKey": "titan_lk",
```

Имя генерируемого типа токена. По умолчанию `ConnectToken`  
```json
"typeName": "ConnectToken"
```

**Что порождается.** Секция разворачивается в обычные метаданные до разбора контроллеров, поэтому дальше работает штатный конвейер. Появляются тип с полями `Id`, `Token`, `ExternalUserId`, `DisplayName`, `ExpiresAt`, `TenantExternalKey`, `ProductExternalKey`, `ApiBaseUrl` (отдельный json-файл для него не нужен), метод с маршрутами чтения, записи и удаления, и функция `IssueConnectToken`. Клиент получает метод `issueConnectToken()`, а вместе с токеном — tenant, ключ продукта и адрес Chatista, так что в приложении их прописывать не нужно.

**Что пишется руками.** Генератор создаёт объявление продуктового хука `ResolveSupportChatUser`, а его реализацию пишет продукт. `externalUserId` обязан быть постоянным: `user.UserId` — это guid токена, он меняется при каждом входе, и человек заводился бы в Chatista заново после каждой авторизации. По этому же идентификатору платформа находит устройства при отправке уведомлений.

**Версия генератора.** Ключ поддерживается начиная с коммита `4df0009`. Сборка старее секцию просто не применит: тип, метод и функция не попадут в вывод, а продукт, который на них рассчитывает, перестанет компилироваться.

Полное описание интеграции — от заведения продукта в Chatista до диагностики — в [отдельном документе](docs/support-chat-integration.md).
...
Предопределенные объекты справочника
```json
"predefinedObjects": [
    {
        "id": "00000000-0000-0000-0000-000000000000",
        "name": "testPredefinedObject",
        "description": "Тестовый предопределенный объект"
    }
],
```
...
[Уровень прав](#уровни-авторизации) для маршрутов этого типа данных. Имеет приоритет над полем `authorize` в [описании метода](#1-1-1-описание-структуры-методов-работы-с-объектами-данных-method) в generation_config. По умолчанию none.  
```json
"authorize": "anonymous",
```
...
| ```Guid``` | идентификатор (на клиенте равнозначен String) |
| ```Reference<T>``` | ссылка на объект |
| ```UntypedReference<T0, T1...>``` | нетипизированная ссылка на объект |
| ```List<T>``` | список объектов (в [метаданных](#объект-метаданных) - табличная часть) |
| ```Enum<T>``` | перечисление |
| ```Image``` | изображение |
...
| - | - | - |
| ```anonymous``` | авторизация пользователя не требуется (достаточно анонимного токена), но будет проверена | `[Authorize]` |
| ```user``` | авторизованный пользователь | `[Authorize(Roles = UserRoles.User)]` |
| ```admin``` | роль администратора | `[Authorize(Roles = UserRoles.Admin)]` |
| ```none``` | без проверки | без аттрибута; значение по умолчанию для метода |
...
По окончании генерации выводится список **реально изменённых файлов** (с полными путями) — файлы, содержимое которых изменилось.
...
В VS Code понадобится установить расширение Dart. Над методом main появятся кнопки Run и Debug.  
